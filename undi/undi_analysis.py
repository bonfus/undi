...
def execute_undi_analysis(
    ...
    """
        ...
        Bmod (float, optional): The external magnetic field magnitude. Default is 0.0.
    ...
    for cluster in product(*info.values()):
        ...
        NS.translate_rotate_sample_vec(direction) # translate_rotate_sample_by_vec
        signal_z_tf = NS.celio_on_steroids(t,  k=4, algorithm=algorithm)
        del NS
        ...
        NS = MuonNuclearInteraction(undi_input,external_field=B_lf)
        NS.translate_rotate_sample_vec(direction) # translate_rotate_sample_by_vec
        signal_x_lf = NS.celio_on_steroids(t,  k=4, algorithm=algorithm)
        del NS
        ### TF
        print("Doing sample along X, TF")
        NS = MuonNuclearInteraction(undi_input,external_field=B_tf)
        NS.translate_rotate_sample_vec(direction) # translate_rotate_sample_by_vec
        signal_x_tf = NS.celio_on_steroids(t,  k=4, algorithm=algorithm)
        del NS
        ...
        NS = MuonNuclearInteraction(undi_input,external_field=B_lf)
        NS.translate_rotate_sample_vec(direction) # translate_rotate_sample_by_vec
        signal_y_lf = NS.celio_on_steroids(t,  k=4, algorithm=algorithm)
        del NS
        ### TF
        print("Doing sample along Y, TF")
        NS = MuonNuclearInteraction(undi_input,external_field=B_tf)
        NS.translate_rotate_sample_vec(direction) # translate_rotate_sample_by_vec
        signal_y_tf = NS.celio_on_steroids(t,  k=4, algorithm=algorithm)
        del NS
