...
def test_fmuf():
    ...
    NS = MuonNuclearInteraction(fmuf, log_level='error')
    NS.translate_rotate_sample_vec([0,0,1])
    signal_FmuF = NS.celio_on_steroids(t, k=2, progress=False)
    for _ in range(999):
        signal_FmuF += NS.celio_on_steroids(t, k=2, progress=False)
    ...
    print(signal_FmuF/3000 - brewer(t, r, gF))
    np.testing.assert_allclose(signal_FmuF/3000, brewer(t, r, gF), atol=2e-2)
